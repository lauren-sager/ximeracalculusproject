...
\title{Intro to Tangent Lines Activity Solution Guide}
...
Label the graphs below with the formula for the function whose graph is shown. Also, determine the domain and range of each function (which might help in identifying the formula). Assume curves extend infinitely beyond the window shown. Finally, label the function as: \textbf{A Power Function, Linear, Quadratic, Polynomial, and/or Rational. List all that apply.}

    \vspace{0.1cm}
    Formula options: $f(x) = \frac{x^3 - 1}{x - 1}$, $f(x) = x^3 - 2x^2 - x + 2$, $f(x) = \frac{1}{x} = x^{-1}$
    \begin{enumerate}
       \item Formula:\textcolor{blue}{$f(x) = \frac{1}{x} = x^{-1}$}\\
        Domain:\textcolor{blue}{$(-\infty,0)\cup(0,\infty)$}\\
        Range:\textcolor{blue}{$(-\infty,0)\cup(0,\infty)$}\\
        Type of Function: \textcolor{blue}{Rational, Power}
        \item Formula: \textcolor{blue}{$f(x) = \frac{x^3 - 1}{x - 1}$}\\
        Domain: \textcolor{blue}{$(-\infty,1)\cup(1,\infty)$}\\
        Range: \textcolor{blue}{$(\infty,0)\cup(0,\infty)$}\\
        Type of Function: \textcolor{blue}{Rational}
        \item Formula: \textcolor{blue}{ $f(x) = x^3 - 2x^2 - x + 2$}\\
        Domain: \textcolor{blue}{All real numbers}\\
        Range:\textcolor{blue}{All real numbers}\\
        Type of Function: \textcolor{blue}{Polynomial, (Rational)}
        \item Formula: \textcolor{blue}{$f(x) = \frac{x^3 - 1}{x - 1}$}\\
        Domain: \textcolor{blue}{$(-\infty,1)\cup(1,\infty)$}\\
        Range: \textcolor{blue}{$(0.75,\infty)$}\\
        Type of Function: \textcolor{blue}{Rational}
    \end{enumerate}

       \begin{image}
        \includegraphics{Tangentintro.jpeg}
    \end{image}
...
\begin{exercise}
  Suppose $f(x)=16-16x^2$.
  \begin{enumerate}
    \item Find an expression for the slope of the secant line on an interval of the form $[0.5, 0.5+h]$ where $0<h<0.5$, or $[0.5+h,0.5]$ when $-0.5<h<0$ and $h\neq 0$.\\
    \textcolor{blue}{m=$\frac{f(0.5+h)-f(0.5)}{0.5+h-(0.5)}=\frac{16-16(0.5+h)^2-16+16(0.5)^2}{h}$
    $=\frac{16-4-16h-16h^2-16+4}{h}=\frac{-16h-16h^2}{h}=\frac{h(-16-16h)}{h}$\\
    $=-16-16h$}
    \item Use this expression to compute the slope of the secant line on $[0.5, 0.75]$ and $[0.4, 0.5]$.\\
    \textcolor{blue}{On $[0.5,0.75]=[0.5,0.5+0.25]$, $h=0.25$\\
    $m=-16-16(0.25)=-16-4=-20$ \\
    On $[0.4,0.5]=[0.5-0.1,0.5]$, $h=-0.1$\\
    $m=-16-16(-0.1)=-16+1.6=-14.4$}
    \item Make a conjecture about the slope of the tangent line at $t=0.5$. \\
    \textcolor{blue}{We can't determine the slope of the tangent line at $t=0.5$ exactly, but we can conjecture that it is between -20 and -14.4 .  }
  \end{enumerate}
