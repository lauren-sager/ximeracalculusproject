...
\title{Infinite Limits and Indeterminate Forms Activity Solutions Guide}
...
%     The one-sided limits do not match, so the limit DNE.}
    ...
    \item $\displaystyle \lim_{x\rightarrow 3} \frac{3-x}{|x-3|}$\\
    \textcolor{blue}{ Gives $\frac{0}{0}$ form\\
    When $x\geq 3$, $|3-x|\leq 0$, so $|3-x|=-(3-x)$.\\
    When $x<3$, $|3-x|>0$, so $|3-x|=3-x$.\\
    $\lim_{x\rightarrow 3^+}\frac{3-x}{|x-3|}=\lim_{x\rightarrow 3^+}\frac{3-x}{-(x-3)}=1$\\
     $\lim_{x\rightarrow 3^-}\frac{3-x}{|x-3|}=\lim_{x\rightarrow 3^-}\frac{3-x}{(x-3)}=-1$\\
    $\lim_{x\rightarrow 3}f(x)$ DNE}
