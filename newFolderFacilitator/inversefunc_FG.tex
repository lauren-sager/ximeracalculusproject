...
\title{Inverse Functions Activity Facilitator Guide}
...
\begin{itemize}
    ...
    \item Find the domain and range of a function and its inverse.
    \item Identify when a function will not have an inverse.
\end{itemize}

\textbf{Prerequisite Knowledge:} This activity is meant to be a review of inverse functions.  The activity assumes students know how to find an inverse and how the domain and range of a function and its inverse are connected.  The very last problem requires the use of inverse trig functions, but this may be omitted if desired.
...
\begin{exercise}
    ...
    \textcolor{orange}{The relationship here is a tricky one to put into words.  If students are stuck (or have forgotten the +/- part of the square root function), have them explore graphs to see what is different between the two functions.  It may help them to remind them that taking an inverse reflects the graph over the line $y=x$.}
\end{exercise}
...
\begin{exercise}
    ...
    \end{enumerate}
    \textcolor{orange}{This function is relatively straightforward, and some students may even have the two versions memorized.  The main goal here is to have students think through what is happening when we compose a function with its inverse and why the output is just the original value.}
    Problem from Boelkins et al., \textit{Active Prelude to Calculus}.
\end{exercise}
