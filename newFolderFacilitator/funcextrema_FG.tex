...
\title{Function Extrema Activity Facilitator Guide}
...
Students should be able to: 
    \begin{itemize}
        \item Sketch a graph reflecting maximum and minimum values of a function.
        \item Find critical numbers of functions.
        \item Find absolute extrema of functions on closed intervals using the closed interval method.
    \end{itemize}

\textbf{Prerequisite Knowledge:} Students need to be able to find derivatives of functions, and solve these derivatives set equal to 0.  Students also need to know where a derivative does not exist.  This activity does not use the first or second derivative test, so students do not need to know these tests to complete the activity.  The final problem expects students to use the closed interval method to find absolute extrema, although a sign diagram with arguments about the functions (increasing, decreasing, etc) is techinically enough to find the absolute extrema.
...
\begin{exercise}
    ...
    \textcolor{orange}{This is the function built from taking the antiderivative of the derivative $f'(x)=-(x-2)(x+2)(x-1)^2x$ that has zeroes where intended.  This makes a kinda of weird looking function, so students' pictures will likely look much nicer.}
\end{exercise}
