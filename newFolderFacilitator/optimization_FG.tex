...
\begin{exercise}
    ...
    \begin{enumerate}
        ...
        \item Sketch the graph of the total cost function.  You may use a graphing calculator or Desmos. 
          \begin{image}
            \begin{tikzpicture}[x=0.5cm, y=0.2cm]
  % Axes
  \draw[-stealth] (0,0) -- (10,0) node[below right] {$x$};
  \draw[-stealth] (0,0) -- (0,30) node[above left] {$y$};

  % Grid (optional)
  \draw[very thin, lightgray] (0,0) grid (10,30);

  % Graph for x>0
  \draw[very thick, blue, smooth, samples=300, domain=0.05:10]
    plot (\x, {0.054*(\pi)*(\x)^2 + 0.48/(\x)});
\end{tikzpicture}
          \end{image}
