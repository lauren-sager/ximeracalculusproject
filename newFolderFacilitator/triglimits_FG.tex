...
\title{Trig Limits and Squeeze Theorem Activity Facilitator Guide}
...
\textbf{Student Learning Outcomes:} 
 Students will be able to: 
    \begin{itemize}
        \item Evaluate limits of trig functions using the facts $\displaystyle\lim_{x\rightarrow 0}\frac{\sin(x)}{x}=1$ and $\lim_{x\rightarrow 0}\frac{\cos(x)-1}{x}=0$.
        \item Evaluate limits involving trig functions.
        \item Apply the squeeze theorem to evaluate limits when the squeeze functions are given.
        \item Determine whether the squeeze theorem can be applied to a given limit.
    \end{itemize}
\textbf{Prerequisite Knowledge:} This activity covers limits of trig functions given that $\displaystyle\lim_{x\rightarrow 0}\frac{\sin(x)}{x}=1$ and $\lim_{x\rightarrow 0}\frac{\cos(x)-1}{x}=0$, and the squeeze theorem (as it is connected to finding these two trig limits). Both of these topics should be covered before this activity.\\
The first exercise is meant to be a review of the algebraic techniques for evaluating trig limits.  The second exercise is meant to be a more straightforward application of the squeeze theorem, while the third exercise is meant to be a more open ended exploration of how the squeeze theorem can be used (or not used) in various situations.  This exercise is meant to encourage students to think critically about how the squeeze theorem works, and to understand that it is not always applicable, but that it can still provide useful information even when it cannot be directly applied.
...
\begin{exercise}
    ...
    \begin{enumerate}
        ...
        \textcolor{orange}{Students tend to want to find an algorithm for these problems based on which numbers appear where.  Try to encourage students to actually go through the process to make sure their thinking is clear.  This is a place where students sometimes try to pull the constant out of the trig function and call it a day.  Make sure students are not using this method by encouraging them to show all of their work.}
        ...
        \textcolor{orange}{This problem is not connected to the trig rules, but here to show students that trig functions can show up in other types of limits. Graphs are good tools here to support students as they work through this problem.}
    \end{enumerate}  
...
\begin{exercise}
  ...
  \begin{enumerate}
    ...
    \textcolor{orange}{All students need to show here is that direct substitution fails.  It may be helpful to ask students if there are any other methods for indeterminate forms that might help here.  Note that we have NOT learned l'Hopital's rule yet.  If students know it, they may check their work, but they should be using the squeeze theorem as outlined in the problem to get their final answer.}
...
\begin{exercise}
    ...
    \textcolor{orange}{This problem reinforces the idea that just because something looks like a squeeze theorem setup, it does not have to be.  Graphs are valuable here as a visualization of the situation for the final part of the question.}
