...
\begin{itemize}
    \item Find the derivative of linear, quadratic and simple cubic polynomials using the derivative definition.
    \item Find the derivative of a function from a graph when the function is linear or at a max or min value.
    \item Show that a derivative does not exist using the limit definition of the derivative.
    \item Identify the sign of a derivative from a graph.
\end{itemize}

\textbf{Prerequisite Knowledge}: It is assumed that students have seen the limit definition of the derivative, although it is also included here for reference.  Students are also expected to be familiar with limits.  \\

This activity is likely too long for most students.  Divide and conquer could be a viable strategy on problem 2, as long as students are getting experience with at least one formula across types of polynomials.  
...
\begin{exercise}
    ...
    \end{enumerate}
    \textcolor{orange}{These locations were all chosen so the slope of the function can be exactly calculated.  If groups are engaged with this problem, you can raise the question of what happens at the points where the pieces of the function connect, and also to estimate slopes along the parabola part of the function.}
\end{exercise}
...
Calculate the following derivatives at the given value using both the $h\rightarrow 0$ and $b\rightarrow a$ limit definitions of the derivative.\\
    \begin{enumerate}
        ...
        \textcolor{orange}{An interesting question to ask yourself if you are used to only one of the definitions!}
    \end{enumerate}
    ...
    \begin{exercise} 
        \begin{enumerate}
            ...
            \textcolor{blue}{$f'(-1)=-5$ (calculated in 2b)\\
                $f(-1)=(-1)^2-3(-1)=4$\\
                Equation of the tangent line: $y-4=-5(x+1)$}\\
            \textcolor{orange}{Note that this value was calculated in 2b and doesn't need to be done again! }
    ...
    \begin{exercise}
        ...
        \textcolor{orange}{This is likely a new idea for most students - thinking about how the limit and derivative work together like this.  Have students set up the definition if they are stuck, and then work from there.  This is another place where it is helpful to remember that a limit of infinity is not a limit that exists.}
    \end{exercise}
