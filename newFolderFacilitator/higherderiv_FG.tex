...
Students will be able to:
    \begin{itemize}
        \item Calculate higher order derivatives of functions.
        \item Identify graphs of functions and their first and second derivatives.
        \item Write equations for tangent lines to derivative functions.
    \end{itemize}

\textbf{Prerequisite Knowledge:} Students are expected to know the rules for calculating derivatives (chain rule is not explicitly required), and what it means to take a higher order derivative.  Students should also have experience with how derivatives connect to the shape of a graph.\\

Question 2 begins as just taking higher order derivatives, but then tries to step students through how we may create a formula for the nth derivative, looking ahead to Calculus II, particularly Taylor Series.  \\

This activity may be on the short side.  The activity can be used on its own as a short recitation activity, or the problems can be added to other activities as reinforcement. 
...
\begin{exercise}
    ...
    $f^{(4)}(x)=24$\\
    $f^{(5)}(x)=0$}\\
    \textcolor{orange}{Students may struggle with the arbitrary coefficients here.  We are trying to get them used to using letters to stand in for constants, as we will see this in the future.}
\end{exercise}
...
\begin{exercise}
    ...
    $f^{(5)(x)}=\frac{105}{32}x^{-\frac{9}{2}}$}
        \begin{enumerate}
            ...
            \textcolor{orange}{As mentioned above, parts (a)-(c) are meant to step students through the thought process of coming up with an nth derivative formula in anticipation of using this skill for Taylor Series in Calc II.  This activity doesn't require students to find the nth derivative, but instead asks them to confirm the pieces in the order an experienced student may look to build the formula.}
        \end{enumerate}
\end{exercise}
...
\begin{exercise}
    ...
    \begin{enumerate}
        ...
        \textcolor{orange}{The easiest way to think about these graphs is to think about zeros and intervals of increasing and decreasing.  These graphs are meant to be challening - this problem definitely involves some higher level thinking. If students are totally stuck, they can start drawing the derivative of each function and see if they can find a match.}
    \end{enumerate}
