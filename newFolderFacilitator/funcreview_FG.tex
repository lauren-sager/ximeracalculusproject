...
\title{Function Review Activity Facilitator Guide}
...
\begin{itemize}
    \item Use functions to describe area and perimeter of a geometric object.
    \item Find and interpret the domain of a function, with our without given context.
    \item Evaluate and sketch piecewise functions.
    \item Classify given functions by type.
\end{itemize}
\textbf{Prerequisite Knowledge:} This activity assumes students have a background with types of functions, domain, and piecewise functions.  These are topics that are expected to be covered before a Calculus I course, but may need refreshing at the beginning of the course to faciliate further use of functions through the semester.
...
\begin{exercise}
 ...
 In this problem, let $x$ denote the length of the base of a rectangle. Suppose the height of the rectangle is always half the length of its base.
    \begin{enumerate}
       ...
       \textcolor{orange}{This problem always trips students up.  I think for many of them, this is the first time they have seen a question like this.  The idea of this problem is to help them solidify their understanding of functions for later problems like related rates or optimization problems.}
    \end{enumerate}
\end{exercise}
...
\begin{exercise}
  ...
  \begin{enumerate}
      ...
      \textcolor{blue}{$P(q)$, $F(t)$, $g(x)$, $C(z)$}
      \item Are any of these functions classified as algebraic functions?  Which one(s)?\\
      \textcolor{blue}{$F(t)$, $P(q)$, $g(x)$, $h(x)$, $H(t)$, $C(z)$}
  \end{enumerate}
  \textcolor{orange}{Algebraic and rational functions tend to trip students up, because they are boarder umbrellas that encompass types of functions students are more confortable.  For example, technically a polynomial is a rational function with denominator $q(x)=1$.  We really want students to understand the idea of a rational function being $\frac{p(x)}{q(x)}$, a quotient of two polynomials, not fixate on this technicality, but some students may get hung up on this.}
\end{exercise}
